function  U  = Controller_v1(y, str_z, awz, h, p_air, Cp,...
...
Uz=(-hh(1:Leng)-itta(1:Leng)-((A(1:Leng,1:Leng)-diag(gain))*Xref')'-(diag(gain)*y(1:Leng)')')/(g(1:Leng,1:Leng));
...
if y(Leng+1)-To <= DTmax
    Ps=1+ (COPmax-1)*(1-(y(Leng+1)-To)/DTmax);
else
    Ps=1;
end
...
U=( -hh -itta -((A-diag(gain))*Xref')' -(diag(gain)*y(1:Leng+1)')' )/g;
end
