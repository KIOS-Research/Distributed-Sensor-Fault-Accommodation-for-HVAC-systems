function  U  = Controller_v3_decentralized(y, str_z, awz, h, p_air, Cp,...
...
for k=1:(Leng)  
    ...
    for c=1:(length(str_z(k).connected))
        ...
        sum2=sum2+(str_z(k).az_ij(c)/str_z(k).cz)*...
            str_z(str_z(k).connected(c)).Tref;%y(str_z(k).connected(c));
...
sum1=0;
for k=1:Leng
    sum1=sum1 + str_z(k).Umax * (y(Leng+1)-y(k))*Uz(k);%(y(Leng+1)-str_z(k).Tref)*Uz(k);
end
...
U=(-y(1:Leng+1)*A-itta+gain.*(Xref-y(1:Leng+1)))/g; %-hh
