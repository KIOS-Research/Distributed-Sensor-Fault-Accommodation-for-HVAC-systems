function  U  = Controller_v4_decentralized(y, x, str_z, awz, h, p_air,...
...
for k=1:(Leng)  
    ...
    g(k,k)=(((str_z(k).Umax * awz)) /(str_z(k).cz)) *...
        (x(2*Leng+2)-y(k));%(y(Leng+1) - y(k));%(Trefw - y(k));
    ...
    for c=1:(length(str_z(k).connected))
        ...
        sum2=sum2+(str_z(k).az_ij(c)/str_z(k).cz)*...
            str_z(str_z(k).connected(c)).Tref;%x(str_z(k).connected(c)+Leng+1); % %y(str_z(k).connected(c));
        for d=1:str_z(k).paths.TotalPaths
            sum3 = sum3 + sign( x(str_z(k).paths.ConnDoors(d)+Leng+1) - y(k) )...
                ...
                * max( y(k) , x(str_z(k).paths.ConnDoors(d)+Leng+1) ) *...
                sqrt(2 * (Cp - Cv) *...
                abs( str_z(str_z(k).paths.ConnDoors(d)).Tref - y(k)) ); %x(str_z(k).paths.ConnDoors(d)+Leng+1)
        end
...
sum1=0;
for k=1:Leng
    sum1=sum1 + str_z(k).Umax * (y(Leng+1)-str_z(k).Tref)*Uz(k);%(y(Leng+1)-x(Lest(k)))*Uz(k);
end
...
U=(-y(1:Leng+1)*A-hh-itta+gain.*(Xref-y(1:Leng+1)))/g;%-hh
